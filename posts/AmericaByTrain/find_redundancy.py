...
def main(index,strt,end,cid,local):
    ...
    for i in index:
        ...
        if len(isegs) > 2: #a linear track should have 2- a split in the track would have 3
            ...
            if len(paths) > 0: paths = np.delete(paths,np.arange(len(isegs)-1),1)
            ...
            if verbose:
                ...
                ir = iredund[-5:]
                print('  ---   ')
                print([cid[i] for i in jiffy])
                print('---------------------')
                stop

####Do again for end indices, just in case there's a juncture of a bunch of ends
    for i in index:
        if (i+1) % 100 == 0: print(i+1,'/',len(index))
        pep = end[i]
        strt_dist = [(pep[0]-p[0])**2+(pep[1]-p[1])**2 for p in strt]
