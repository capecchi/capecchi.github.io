...
def main(newdata=False):
    ...
    eb_ends = [[-93.070892,44.949035],[-122.331837,47.587021]] #[St. Paul, Seattle]
    ...
    if newdata:
        ...
        while building:
            ...
            i = 0
            while scanning:
                cc = feat[i]['geometry']['coordinates']
                print(findpt)
                print(cc)
                stop
                if findpt in cc:
                    scanning = 0
                    if findpt != cc[0]: cc = cc[::-1]
                    for pair in cc: eb_coords.append(pair)
                    findpt = cc[-1]
                if stoppt in cc:
                    building = 0
                i += 1
