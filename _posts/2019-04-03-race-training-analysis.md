...
Included in the sweat rate analysis, since I have to record my weight before and after a run, is a history of my weight. I've been using this as another marker of race-readiness, plotting weight over time with race dates overlaid. However, I've been tracking this data in a notes app on my phone until I run the python code to pull my Strava data, at which time I update my spreadsheet with which shoes I wore and how much I weighed after each run. Huge pain since weighing myself doesn't need to be associated with a run (only if I'm trying to analyze sweatrate) and might sit in my phone for weeks if I don't run this analysis to pull my data. So as a fun exercise I implemented the following workflow:

- I created an Alexa skill in the developer console called "Weight Logger" that when invoked by the phrase "Alexa, ask weight logger to record 180 pounds" will transmit the weight value to an AWS lambda function
- The lambda function then runs some short python code to append it with a timestamp to a weights.json file in my github repository
- On github, I have a workflow set up via a .yml file that checks for changes to weights.json and if detected, sets up a virtual machine, runs python code to update my weight history figure, and pushes the changes to my repository

So now I only have to weigh myself and tell the robot, and my figure will automatically be updated! Up until this update I haven't shown this figure, and I anticipate nobody will care how much I weigh, but this is basically nothing more than a digital journal anyway, and I thought the workflow was really cool so I wanted to share :)

<iframe src="/images/posts/rta_weighthist.html" height="500" width="100%"></iframe>
